...
<div class="row">
    <div class="col-12">
      <div class="card">
        ...
        <!-- /.card-header -->
        <div class="card-body">
          <form action="{{ route('admin.binhluan.show',$id) }}" method="get">
            {{-- {{$id}} --}}
            @csrf
            <div class="row">
              <div class="col-md-4">
                  <input type="text" class="form-control" name="search" placeholder="Tìm kiếm tên sản phẩm hoặc nội dung" value="{{ request('search') }}">
              </div>
              <div class="col-md-3">
                  <select class="form-control" name="trang_thai">
                      <option value="">Tất cả trạng thái</option>
                      <option value="1" {{ request('trang_thai') == 1 ? 'selected' : '' }}>Hiển thị</option>
                      <option value="2" {{ request('trang_thai') == 2 ? 'selected' : '' }}>Ẩn</option>
                  </select>
              </div>
                    ...
                    <td class="{{ $item->trang_thai === 1  ? "text-success" :  "text-danger" }}">{{ $item->trang_thai === 1  ? "Hiển thị" :  "Ẩn" }}</td>
                     <td>
                        ...
                        <div style="display:flex; column-gap:5px ;justify-content: center; ">   
                            <a href="{{ route('admin.binhluan.edit',$item->binh_luan_id) }}" class="btn btn-info">Sửa</a>
