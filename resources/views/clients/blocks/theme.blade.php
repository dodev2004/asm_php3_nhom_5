<script>
    ...
    let UomoHelpers = {
        ...
        showPageBackdrop: () => {
            UomoSelectors.$pageBackDrop && UomoSelectors.$pageBackDrop.classList.add(UomoSelectors
                .pageBackDropActiveClass);
        ...
        hidePageBackdrop: () => {
            UomoSelectors.$pageBackDrop && UomoSelectors.$pageBackDrop.classList.remove(UomoSelectors
                .pageBackDropActiveClass);
    ...
    (function() {
        ...
        UomoElements.QtyControl = (function() {
            function QtyControl() {
                // Gán sự kiện click cho phần tử cha chứa các điều khiển số lượng
                document.addEventListener('click', function(event) {
                    const $reduce = event.target.closest('.qty-control__reduce');
                    const $increase = event.target.closest('.qty-control__increase');
                    if ($reduce || $increase) {
                        // Xác định phần tử điều khiển số lượng cha
                        const $qty = $reduce ? $reduce.closest(UomoSelectors.quantityControl) :
                            $increase.closest(UomoSelectors.quantityControl);
                        if (!$qty) return;
        ...
        UomoElements.Search = (function() {
            ...
            Search.prototype = Object.assign({}, Search.prototype, {
                    ...
                    this.$containers.forEach(el => {
                        ...
                        $inputBox && $inputBox.addEventListener('keyup', (e) => {
                            const filterValue = e.currentTarget.value
                                .toUpperCase();
                            el.querySelectorAll(_this.selectors
                                .searchSuggestItem).forEach(el => {
                         ...
                         */
                        el.querySelectorAll(_this.selectors.searchCategorySelector)
                            .forEach(scs => {
                                scs.addEventListener('click', function(e) {
                                    e.preventDefault();
                                    const $s_f_a = el.querySelector(_this
                                        .selectors.searchFieldActor);
                                    if ($s_f_a) {
                ...
                _initSearchReset: function() {
                    ...
                    document.querySelectorAll(this.selectors.resetButton).forEach(el => {
                        el.addEventListener('click', function(e) {
                            const $parentDiv = e.target.closest(_this.selectors
                                .container);
                            const $inputBox = $parentDiv.querySelector(_this
                                .selectors.inputBox);
                            const $rc = $parentDiv.querySelector(_this.selectors
                                .resultContainer);
                ...
                _updateSearchResult: function(data, $form) {
                    ...
                    $form.querySelector(this.selectors.resultContainer).innerHTML = $f_r
                        .innerHTML;
            ...
            Aside.prototype = Object.assign({}, Aside.prototype, {
                _init: function() {
                    ...
                    $(document).on("click", this.selectors.activator, function(e) {
                        ...
                        $targetAside && $targetAside.classList.add(_this.selectors
                            .activeClass);
                    });
                },
        ...
        UomoElements.Countdown = (function() {
            ...
            Countdown.prototype = Object.assign({}, Countdown.prototype, {
                _init: function() {
                    const _this = this;
                    const $countdowns = this.$container.querySelectorAll(this.selectors
                        .element);
            ...
            ShopViewChange.prototype = Object.assign({}, ShopViewChange.prototype, {
                ...
                _changeViewCols(parentId, newCol) {
                    ...
                    if ($targetDom) {
                        $targetDom.classList.remove(
                            'row-cols-xl-2', 'row-cols-xl-3', 'row-cols-xl-4', 'row-cols-xl-5',
                            'row-cols-xl-6',
                ...
                _resetActiveLinks() {
                    ...
                    document.querySelectorAll(
                        `${this.selectors.element}.${this.selectors.activeClass}`).forEach(
                        $el => {
            ...
            StickyElement.prototype = Object.assign({}, StickyElement.prototype, {
                _init: function() {
                    ...
                    this.$stickies.forEach(function($sticky) {
                        const $grid = $sticky.previousElementSibling || $sticky
                            .nextElementSibling;
                        const $target = $grid.offsetHeight > $sticky.offsetHeight ?
                            $sticky : $grid;
                        ...
                        UomoHelpers.sideStkEl.initialTopOffset = parseInt(window
                ...
                _updateStatus() {
                    ...
                    _this.$stickies.forEach(function($sticky) {
                        ...
                        var bounds = $target.getBoundingClientRect(),
                            maxTop = bounds.top + window.scrollY - $target.offsetTop +
                        ...
                        if (window.scrollY < $target.lastKnownY) {
                            UomoHelpers.sideStkEl.currentTop -= window.scrollY - $target
                                .lastKnownY;
                        } else {
                            UomoHelpers.sideStkEl.currentTop += $target.lastKnownY - window
                                .scrollY;
                        }


                        UomoHelpers.sideStkEl.currentTop = Math.min(Math.max(UomoHelpers
                                .sideStkEl.currentTop, -minTop), maxTop, UomoHelpers
                            .sideStkEl.initialTopOffset);
                        $target.lastKnownY = window.scrollY;
        ...
        UomoSections.Header = (function() {
            ...
            Header.prototype = Object.assign({}, Header.prototype, {
                _init: function() {
                    const headerClass = UomoHelpers.isMobile ? this.selectors.mobileHeader :
                ...
                _initMobileMenu: function() {
                    const _this = this;
                    const mobileMenuActivator = this.$header.querySelector(this.selectors
                        .mobileMenuActivator);
                    const $mobileDropdown = this.$header.querySelector(this.selectors
                        .mobileMenu);
                    let transformLeft = 0;
                    if ($mobileDropdown) {
                        mobileMenuActivator && $(mobileMenuActivator).off('click').on('click',
                            function(event) {
                                event.preventDefault();
                                if (document.body.classList.contains(_this.selectors
                                        .mobileMenuActiveClass)) {
                                    document.body.classList.remove(_this.selectors
                                        .mobileMenuActiveClass);
                                    _this.$header.style.paddingRight = '';
                                    $mobileDropdown.style.paddingRight = '';
                                } else {
                                    document.body.classList.add(_this.selectors
                                        .mobileMenuActiveClass);
                                    _this.$header.style.paddingRight = UomoSelectors
                                        .scrollWidth;
                                    $mobileDropdown.style.paddingRight = UomoSelectors
                                        .scrollWidth;
                                }
                            });
                        ...
                        $mobileMenu && $mobileMenu.querySelectorAll(_this.selectors
                            .mobileSubNavOpen).forEach($btn => {
                            $btn.addEventListener('click', function(event) {
                                event.preventDefault;
                                $btn.nextElementSibling.classList.remove(_this
                                    .selectors.mobileSubNavHiddenClass);

                                transformLeft -= 100;
                                if (menuMaxHeight < $btn.nextElementSibling
                                    .offsetHeight) {
                                    $mobileMenu.style.transform =
                                        'translateX(' + transformLeft
                                        .toString() + '%)';
                                    $mobileMenu.style.minHeight = $btn
                        ...
                        $mobileMenu && $mobileMenu.querySelectorAll(_this.selectors
                            .mobileSubNavClose).forEach($btn => {
                            $btn.addEventListener('click', function(event) {
                                event.preventDefault;
                                transformLeft += 100;
                                $mobileMenu.style.transform = 'translateX(' +
                                    transformLeft.toString() + '%)';
                                $btn.parentElement.classList.add(_this.selectors
                                    .mobileSubNavHiddenClass);
                                const $wrapper = $btn.closest('.sub-menu');
                                if ($wrapper) {
                                    const minHeight = menuMaxHeight < $wrapper
                                        .offsetHeight ? $wrapper.offsetHeight :
                                        menuMaxHeight;
                                    $mobileMenu.style.minHeight = minHeight +
                                        'px';
                                }
                            });
                        });
                ...
                _initStickyHeader: function() {
                    ...
                    if (this.$header.classList.contains("header-transparent-bg")) {
                        ...
                        if (document.querySelectorAll(".header-transparent-bg .header-top")
                            .length > 0) {
                            headerHeight = document.querySelector(
                                ".header-transparent-bg .header-top").offsetHeight;
                        }
                    }
                ...
                _setBoxMenuPosition(menu) {
                    ...
                    const limitL = 0;
                    const menuPaddingLeft = parseInt(window.getComputedStyle(menu, null)
                        .getPropertyValue('padding-left'));
                    const parentPaddingLeft = parseInt(window.getComputedStyle(menu
                        .previousElementSibling, null).getPropertyValue('padding-left'));
                    const centerPos = menu.previousElementSibling.offsetLeft - menuPaddingLeft +
                        parentPaddingLeft;
                ...
                _stickyScrollHander() {
                    ...
                    if (currentScrollTop > this.lastScrollTop || currentScrollTop < this
                        .stickyMinPos) {
        ...
        UomoSections.CartDrawer = (function() {
            function CartDrawer() {
                ...
                setTimeout(() => {
                    ...
                    this._initCartItemRemoval(); // Khởi tạo xử lý xóa mục
                }, 1000);
            }
            ...
            CartDrawer.prototype = Object.assign({}, CartDrawer.prototype, {
                _initCartItemsList: function() {
                    ...
                    if (this.$list) {
                        this.$list.style.maxHeight = drawerHeight - headerHeight -
                            actionsHeader - this.asideContentMargin * 2 + 'px';
                    }
                },

                _initCartItemRemoval: function() {
                    // Sử dụng event delegation để xử lý việc xóa mục
                    this.$aside.addEventListener('click', (event) => {
                        if (event.target.matches(this.selectors.cartItemRemover)) {
                            event.preventDefault();
                            const $parentEl = event.target.closest('.cart-drawer-item');
                            const $list = $parentEl.parentElement;
                            console.log($list);
                            // Tìm phần tử cha có class '.cart-item'
                            const $divider = $parentEl.nextElementSibling;
                            if ($parentEl) {
                                $parentEl.classList.add('_removed');
                                if ($divider && $divider.classList.contains(
                                        'cart-drawer-divider')) {
                                ...
                                setTimeout(() => {
                                    ...
                                    const url = location.origin +
                                        "/client/cart/delete/" + id;
                                    const csrf = event.target.querySelector(
                                ...
                                }, 350); // Thay đổi thời gian nếu cần
                            }
                        }
                    });
                }
            ...
            SwiperSlideshow.prototype = Object.assign({}, SwiperSlideshow.prototype, {
                _initSliders() {
                    this.$containers.forEach(function($sliderContainer) {
                        if ($sliderContainer.classList.contains(
                        ...
                        if ($sliderContainer.dataset.settings) {
                            settings = Object.assign(settings, JSON.parse($sliderContainer
                                .dataset.settings));
                        }
                        ...
                        if ($sliderContainer.querySelectorAll('.swiper-slide').length > 1) {
                            ...
                            const $active_slide = $sliderContainer.querySelector(
                                '.swiper-slide');
                            $active_slide && $active_slide.classList.add(
                                'swiper-slide-active');
                        }
                    });
                }
                                ...
                                ".product-single__image .swiper-container", {
                                    ...
                                    on: {
                                        slideChangeTransitionStart: function() {
                                            galleryThumbs.slideTo(galleryMain
                                                .activeIndex);
                                        }
                                    }
                                });
        ...
        class Uomo {
            constructor() {
                ...
                this.initCartItemRemoval(); // Thêm phương thức xử lý xóa cart
            ...
            initCookieConsient() {
                ...
                getCookie("purecookieDismiss") || (appendHtml(document.body,
                ...
                $containers.forEach(el => {
                    ...
                    $selectArray.forEach(el => {
                        el.addEventListener('click', function(e) {
                            ...
                            const optionIndex = (Array.prototype.indexOf.call($list
                                .children, e.currentTarget)).toString();
                            ...
                            if ($actor && !$actor.classList.contains('js-no-update')) {
                                ...
                                if ($select.selectedIndex > -1) {
                                    content = '';
                                    for (let i = 0; i < $select.selectedOptions
                                        .length; i++) {
                ...
                document.querySelectorAll(selectors.elementClass).forEach($se => {
                    ...
                    if ($se) {
                        ...
                        priceRange.on('slideStop', (value) => {
                            const $minEl = $se.parentElement.querySelector(selectors
                                .minElement);
                            const $maxEl = $se.parentElement.querySelector(selectors
                                .maxElement);
            ...
            initCartItemRemoval() {
                ...
                document.addEventListener('click', (event) => {
                    if (event.target.matches('.js-cart-item-remove')) {
                        event.preventDefault();
                        const $parentEl = event.target.closest('.cart-drawer-item');
                        const $divider = $parentEl.nextElementSibling;
                        console.log(123123);
                        if ($parentEl) {
                            $parentEl.classList.add('_removed');
                            if ($divider && $divider.classList.contains('cart-drawer-divider')) {
                                $divider.classList.add('_removed');
                            }
                            setTimeout(() => {
                                $parentEl.remove();
        ...
        $('a[data-bs-toggle="tab"]').on('shown.bs.tab', function(e) {
            ...
            if ($thePane.find('.swiper-container').length > 0 && 0 === $thePane.find('.swiper-slide-active')
                .length) {
                document.querySelectorAll('.tab-pane' + paneTarget + ' .swiper-container').forEach(function(
                    item) {
                    item.swiper.update();
        ...
        $('#quickView.modal').on('shown.bs.modal', function(e) {
            ...
            if ($thePane.find('.swiper-container').length > 0 && 0 === $thePane.find('.swiper-slide-active')
                .length) {
                document.querySelectorAll('.modal' + paneTarget + ' .swiper-container').forEach(function(
                    item) {
                    item.swiper.update();
                    item.swiper.lazy.load();
                });
        ...
        $(document).on('click', '.cart-table .remove-cart', function(e) {
            ...
            setTimeout(() => {
                const id = e.target.closest(".remove-cart").dataset.index;
                const url = location.origin + "/client/cart/delete/" + id;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $(parentEl).remove();
                        const $list = document.querySelector(".cart-table");

                        console.log(response);
                        let $total = 0;
                        const $items = $list.querySelectorAll(
                            ".shopping-cart__subtotale");
                        ...
                        const totalprice = document.querySelector(".cart_subtotal");
        ...
        $(window).on("scroll", function() {
            if ($(".mobile_fixed-btn_wrapper").length > 0) {
                if ($(this).width() < 992 && $(this).width() >= 768) {
                    if ($(this).scrollTop() + $(this).height() - 76 <= $(".mobile_fixed-btn_wrapper")
                        .offset().top && $(this).scrollTop() > $(this).height()) {
                    ...
                    if ($(this).scrollTop() + $(this).height() - 124 <= $(".mobile_fixed-btn_wrapper")
                        .offset().top && $(this).scrollTop() > $(this).height()) {
                        $(".mobile_fixed-btn_wrapper > .button-wrapper").addClass("fixed-btn");
                    } else {
                        $(".mobile_fixed-btn_wrapper > .button-wrapper").removeClass("fixed-btn");
            ...
            .forEach(function(form) {
                form.addEventListener('submit', function(event) {
                    ...
                    form.querySelectorAll("input[data-cf-pwd]").forEach(function(el) {
                        if (form.querySelector(el.getAttribute("data-cf-pwd")).value != el
                            .value) {
                            event.preventDefault();
                            event.stopPropagation();
                        }
                ...
                form.querySelectorAll("input[data-cf-pwd]").forEach(function(el) {
                    el.addEventListener('keyup', function(event) {
                        if (!el.value || form.querySelector(el.getAttribute("data-cf-pwd"))
                            .value != el.value) {
                    ...
                    form.querySelector(el.getAttribute("data-cf-pwd")).addEventListener('keyup',
                        function(event) {
                            if (!el.value || form.querySelector(el.getAttribute("data-cf-pwd"))
                                .value != el.value) {
                                el.classList.add("is-invalid");
